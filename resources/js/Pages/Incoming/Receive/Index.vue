...
<template>
    <AppLayout>
        ...
        <div class="mb-6">
            <h1 class="text-2xl font-bold tracking-tight text-ink-primary">Receive</h1>
            <p class="mt-1 text-sm text-ink-secondary">Arrival yang masih punya sisa qty untuk diterima (FIFO tag).</p>
        </div>
            ...
            <table class="min-w-full divide-y divide-borderline text-sm">
                <thead class="bg-background">
                    <tr class="text-left text-xs font-semibold uppercase tracking-wide text-ink-secondary">
                        ...
                        <th class="px-4 py-3 text-right">Items Pending</th>
                    ...
                    <tr v-for="a in pendingArrivals" :key="a.id" class="hover:bg-primary-light/40">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex justify-end gap-1.5">
                                <Link :href="route('incoming-arrivals.show', a.id)" class="rounded-md bg-primary px-3 py-2 text-xs font-semibold text-white transition hover:bg-primary-hover">Terima</Link>
                            </div>
                        </td>
                    </tr>
                    <tr v-if="!pendingArrivals.length">
                        <td colspan="6" class="px-4 py-12 text-center text-sm text-ink-secondary">Tidak ada arrival pending.</td>
                    </tr>
                </tbody>
            </table>
        </div>
