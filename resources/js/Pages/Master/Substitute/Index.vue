...
function remove(s: PartSubstitute) {
    if (confirm(t('master.deleteSubstitute'))) router.delete(route('substitutes.destroy', s.id));
}
</script>
...
<template>
    <AppLayout>
        ...
        <div class="mb-6 flex items-end justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-ink-primary">{{ t('master.substituteTitle') }}</h1>
                <p class="mt-1 text-sm text-ink-secondary">{{ t('master.substituteSubtitle') }}</p>
            </div>
        ...
        <Modal :show="showForm" max-width="lg" @close="closeForm">
            <form @submit.prevent="submit" class="p-6">
                <h2 class="mb-4 text-base font-semibold text-ink-primary">{{ editing ? t('master.editSubstitute') : t('master.addSubstitute') }}</h2>
                ...
                <div class="mt-4 flex items-center justify-end gap-3">
                    ...
                    <button type="submit" :disabled="form.processing" class="rounded-md bg-primary px-4 py-2 text-sm font-semibold text-white transition hover:bg-primary-hover disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-1 focus-visible:ring-offset-surface">{{ form.processing ? t('master.saving') : t('master.save') }}</button>
                </div>
            </form>
        </Modal>
    </AppLayout>
