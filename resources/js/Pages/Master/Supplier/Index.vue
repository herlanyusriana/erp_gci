...
function remove(s: Supplier) {
    if (confirm(`Hapus supplier ${s.supplier_code}?`)) {
...
<template>
    <AppLayout>
        ...
        <div class="mb-6 flex items-end justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-ink-primary">Supplier Master</h1>
                <p class="mt-1 text-sm text-ink-secondary">Master pemasok — direferensikan lewat supplier_id, bukan string bebas.</p>
            </div>
            <button
                @click="openCreate"
        ...
        <!-- Search -->
        <input v-model="search" @input="doSearch" type="search" placeholder="Cari supplier…" class="mb-4 w-full rounded-lg border-borderline bg-surface px-3 py-2 text-sm text-ink-primary focus:border-primary focus:ring-primary sm:w-80" />
            ...
            <table class="min-w-full divide-y divide-borderline text-sm">
                ...
                <tbody class="divide-y divide-borderline">
                    <tr v-for="s in suppliers.data" :key="s.id" class="hover:bg-primary-light/40">
                        ...
                        <td class="px-4 py-3">
                            <span :class="s.is_active ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning'" class="rounded-md px-2 py-0.5 text-xs font-semibold">{{ s.is_active ? 'ACTIVE' : 'INACTIVE' }}</span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex justify-end gap-1.5">
                                <ActionButton label="Edit" variant="edit" @click="openEdit(s)" />
                                <ActionButton label="Hapus" variant="delete" @click="remove(s)" />
                            </div>
                        </td>
                    </tr>
                    <tr v-if="suppliers.data.length === 0">
                        <td colspan="4" class="px-4 py-12 text-center text-sm text-ink-secondary">Tidak ada supplier.</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <Modal :show="showForm" max-width="md" @close="closeForm">
            <form @submit.prevent="submit" class="p-6">
                <h2 class="mb-4 text-base font-semibold text-ink-primary">{{ editing ? 'Edit Supplier' : 'Tambah Supplier' }}</h2>
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input v-model="form.supplier_name" type="text" placeholder="Nama supplier" class="mt-1 w-full rounded-lg border-borderline bg-surface px-3 py-2 text-sm text-ink-primary focus:border-primary focus:ring-primary" />
                        <InputError :message="form.errors.supplier_name" class="mt-1" />
                    </div>
                </div>
                <div class="mt-4">
                    <label class="text-xs font-semibold text-ink-secondary">Tanda Tangan (gambar)</label>
                    <input
                        type="file"
                        accept="image/*"
                    ...
                    <div v-if="editing?.signature_url || form.signature" class="mt-2 flex items-center gap-3 rounded-lg border border-borderline bg-background p-2">
                        ...
                        />
                        <span class="text-xs text-ink-secondary">Tanda tangan yang akan dipakai pada dokumen invoice (SIGNED BY).</span>
                    </div>
                </div>
