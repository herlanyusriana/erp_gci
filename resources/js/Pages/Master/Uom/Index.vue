...
function remove(u: Uom) {
    if (confirm(`Hapus UOM ${u.code}?`)) router.delete(route('uoms.destroy', u.id));
}
</script>
...
<template>
    <AppLayout>
        ...
        <div class="mb-6 flex items-end justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-ink-primary">UOM Master</h1>
                <p class="mt-1 text-sm text-ink-secondary">Unit of Measure — PCS, KGM, SHEET, ROLL…</p>
            </div>
            <button
                @click="openCreate"
        ...
        <input v-model="search" @input="doSearch" type="search" placeholder="Cari UOM…" class="mb-4 w-full rounded-lg border-borderline bg-surface px-3 py-2 text-sm text-ink-primary focus:border-primary focus:ring-primary sm:w-80" />
            ...
            <table class="min-w-full divide-y divide-borderline text-sm">
                ...
                <tbody class="divide-y divide-borderline">
                    <tr v-for="u in uoms.data" :key="u.id" class="hover:bg-primary-light/40">
                        ...
                        <td class="px-4 py-3">
                            <span :class="u.is_active ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning'" class="rounded-md px-2 py-0.5 text-xs font-semibold">{{ u.is_active ? 'ACTIVE' : 'INACTIVE' }}</span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex justify-end gap-1.5">
                                <ActionButton label="Edit" variant="edit" @click="openEdit(u)" />
                                <ActionButton label="Hapus" variant="delete" @click="remove(u)" />
                            </div>
                        </td>
                    </tr>
                    <tr v-if="uoms.data.length === 0">
                        <td colspan="4" class="px-4 py-12 text-center text-sm text-ink-secondary">Tidak ada UOM.</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <Modal :show="showForm" max-width="md" @close="closeForm">
            <form @submit.prevent="submit" class="p-6">
                <h2 class="mb-4 text-base font-semibold text-ink-primary">{{ editing ? 'Edit UOM' : 'Tambah UOM' }}</h2>
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input v-model="form.name" type="text" placeholder="Pieces" class="mt-1 w-full rounded-lg border-borderline bg-surface px-3 py-2 text-sm text-ink-primary focus:border-primary focus:ring-primary" />
