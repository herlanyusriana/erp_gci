...
**Acceptance criteria:**
- [x] New user-facing strings exist in balanced id/en/ko catalogs and pass the i18n guard.
- [x] TypeScript interfaces cover summaries, filters, event payload, and connection state without `any` escapes.
- [x] Detail and print links remain available while no cancel/update/delete control is rendered.

**Verification:**
- [x] Tests pass: `php artisan test --compact tests/Feature/MaterialIssueWebTest.php`
- [x] Build succeeds: `npm run build`
- [ ] Manual check: switch locale, open detail, print a bon, and confirm no mutation action is present.
      -> BELUM diuji manual: ganti locale + cetak bon belum diverifikasi di browser.
...
- [x] Tests pass: `php artisan test --compact tests/Feature/MaterialIssueWebTest.php`
- [x] Build succeeds: `npm run build`
- [x] Manual check: follow the local setup from a clean terminal and validate every VPS command is executable or clearly marked as a placeholder.
      -> Terlampaui: setup dijalankan langsung di VPS produksi (nooneasku.online), bukan hanya lokal.
...
- [ ] Tests pass: `php artisan migrate:fresh --seed --force && vendor/bin/pest`
      -> BELUM dijalankan dengan `migrate:fresh` (destruktif, DB dev kosong). Sebagai gantinya
         `php artisan test --compact` dijalankan: 131 passed, 744 assertions.
- [x] Build succeeds: `npm run build`
- [x] Manual check: complete the PRD success-criteria checklist and perform the browser real-time flow.
      -> Terbukti di VPS: event release APK mengalir lewat queue -> Reverb -> dashboard live.
...
- [x] Application and frontend build cleanly.
- [x] Query payload, permission boundary, and dependency versions reviewed by a human.

## Checkpoint: After Tasks 4-5

- [x] API release broadcasts once after commit.
- [x] Rollback and idempotency tests pass.
- [x] Private channel denies unauthorized users.

## Checkpoint: After Tasks 6-8

- [x] Dashboard works end-to-end in a browser.
- [x] Live event, filtering, deduplication, reconnect, detail, and print are verified.

## Checkpoint: After Tasks 9-10

- [x] VPS runbook and environment template are safe and complete.
- [x] Full quality gates and PRD acceptance criteria pass.
- [x] Ready for code review and later shipping planning.

## Penutupan Plan (2026-09-23)

Plan ini ditutup. Ringkasan bukti:

- Suite: `php artisan test --compact` -> 131 passed, 744 assertions.
- Build: `npm run build` lulus (termasuk guard i18n).
- Realtime produksi: terbukti live di VPS `nooneasku.online` (release APK -> queue ->
  Reverb :8090 -> dashboard monitoring).
- Reverb di VPS berjalan sebagai `erp-gci-reverb` + `erp-gci-worker` (systemd).

Dua item sengaja TIDAK dicentang karena belum dijalankan apa adanya:

1. Task 8 manual: ganti locale + cetak bon belum diuji manual di browser.
2. Task 10: `migrate:fresh --seed --force` tidak dijalankan (destruktif); diganti
   suite penuh tanpa fresh.

Keduanya bukan blocker dan dapat diverifikasi kapan saja di lingkungan yang aman.
